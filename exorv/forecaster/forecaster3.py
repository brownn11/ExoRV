# essentially the radius to mass estimate from forecaster (Chen 2016) but updated for Python 3
...
## function
from forecaster import func
#from func import ProbRGivenM, classification
...
def Rpost2M(radius, unit='Earth', grid_size=1e3, classify='No'):
	...
	logm_grid = np.linspace(-3.522, 5.477, int(grid_size), dtype=float)

	for i in range(sample_size):
		prob = func.ProbRGivenM(logr[i], logm_grid, hyper[i, :])
		logm[i] = np.random.choice(logm_grid, size=1, p=prob)

	mass_sample = 10. ** logm

	if classify == 'Yes':
		func.classification(logm, hyper[:, -3:])
